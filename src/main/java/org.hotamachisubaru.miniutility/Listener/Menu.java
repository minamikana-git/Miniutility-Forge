...
public class Menu {
    ...
    @SubscribeEvent
    public static void handleInventoryClick(PlayerContainerEvent event) {
        if (!(event.getEntity() instanceof ServerPlayer player)) return;

        if (event instanceof PlayerInteractEvent.RightClickItem rightClickEvent) {
            var clickedItem = rightClickEvent.getItemStack();

            if (clickedItem.isEmpty() || clickedItem.getItem() == null) return;

            String title = PlainTextComponentSerializer.plainText()
                    .serialize(Component.text(rightClickEvent.getHand().name())).trim();
    ...
    private static void handleUtilityBox(ServerPlayer player, net.minecraft.world.item.ItemStack clickedItem) {
        ...
        if (itemType == net.minecraft.world.item.Items.ARMOR_STAND) {
            ...
            if (currentMode.isSurvival()) {
                player.setGameMode(net.minecraft.world.entity.player.PlayerGameMode.CREATIVE);
                player.displayClientMessage(Component.text("ゲームモードをクリエイティブに変更しました。").color(NamedTextColor.GREEN), false);
            } else {
                player.setGameMode(net.minecraft.world.entity.player.PlayerGameMode.SURVIVAL);
    ...
    private static void teleportToDeathLocation(ServerPlayer player) {
        // Implement getting death location and teleport logic for Forge.
        ...
        player.teleportTo(deathLocation.pos().x, deathLocation.pos().y, deathLocation.pos().z);
